/*
请求上下文：解析 From/Via 得到 DeviceID、源地址与对端 Address，驱动中间件链并最终写回 SIP 响应。
*/
...
func newContext(req *Request, tx *Transaction) *Context {
	...
	if err := c.parserRequest(); err != nil {
		sipLog.Error("sip context: parserRequest", "error", err)
	}
	return &c
}
...
func (c *Context) parserRequest() error {
	...
	if c.To == nil {
		sipLog.Error("sip context: to is nil", "header", header)
	}
