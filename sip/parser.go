/*
SIP ABNF 解析：将字节流解析为 Request/Response，含各标准头域的专用解析器与通用 GenericHeader 回退。
*/
...
func (p *parser) start() {
	...
	for !p.isStop {
		...
		if err != nil {
			if err != io.EOF {
				sipLog.Error("start nextLine", "error", err, "line", startLine)
			}
			continue
		}
		if len(startLine) == 0 {
		...
		if isRequest(startLine) {
			...
			sipLog.Error("start unknown", "line", startLine)
			continue
		}
		if termErr != nil {
			sipLog.Error("start termErr", "error", termErr, "line", startLine)
			continue
		}
		...
		flushBuffer := func() {
			if buffer.Len() > 0 {
				...
				if err == nil {
					...
					sipLog.Error("start flushBuffer", "error", err, "line", buffer.String())
				}
				buffer.Reset()
			}
		}
